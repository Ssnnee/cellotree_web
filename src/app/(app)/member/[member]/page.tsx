...
export default function  TreePage( { params }: { params: { member: string } } ) {
  const { treeMember } = MembrHook(params.member)
  const member = treeMember.data?.member

  const getTree = api.tree.getById.useQuery({ id: params.member })
  ...
  return (
    <div className="container relative">
      <PageHeader>
        <PageHeaderHeading>
          Découvrez les enfants du {treeName}
        </PageHeaderHeading>
        <PageHeaderDescription>
